package com.example.hoboirot;

import android.graphics.Color;

public class Const {
    public static final int NR_CATS = 4;

    public static final String[] HOBFREQTIT = {"This Week: ", "This Month: ", "Weekly avg.: ", "Monthly avg.: "};

    public static final Color[] STVW_CLRS = {
            Color.valueOf(0xFFCEDBB6),
            Color.valueOf(0xFFBAB6DB),
            Color.valueOf(0xFFDBB6CC),
            Color.valueOf(0xFFA0A6FF),
            Color.valueOf(0xFFFFA0A0),
            Color.valueOf(0xFFFFB8A0),
            Color.valueOf(0xFFFFEAA0),
            Color.valueOf(0xFFA4E3DD),
            Color.valueOf(0xFFC7C4B2),
            Color.valueOf(0xFFB2C7B4)
    };

    public static final Color[] STVW_CAT_CLRS = {
            Color.valueOf(0xAB543E57),
            Color.valueOf(0xAB3E4357),
            Color.valueOf(0xAB42573E),
            Color.valueOf(0xAB6A553F)
    };
}
